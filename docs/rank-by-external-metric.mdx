...
Go to [everyrow.io/app](https://everyrow.io/app), upload a CSV of the top 300 PyPI packages (with `package` and `monthly_downloads` columns), and enter:
...
[Add the everyrow connector if you haven't already.](/docs/claude-ai) Then upload a CSV of the top 300 PyPI packages (with `package` and `monthly_downloads` columns) and ask Claude:
